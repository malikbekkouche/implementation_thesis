...
class Main {
	public static void main(String[] args){
		...
		Thread t2=new Thread(() -> {
			...
			//while(true){
				...
				for(int j=0;j<10000;j++){
					...
					if(x==null)
						System.out.println("snap get " + j +" is null");
					else if(x!=j)
						System.out.println("snap get " + j +" is not x "+x);
		...
		for(int j=0;j<threadCount;j++){
			t[j]=new Thread(() -> {
				...
				for(int v=0;v<10000;v++){
					...
					//System.out.println("tree remove "+x +" "+tree.remove(x));
					//x=r.nextInt(1000);
					//tree.put(x,x+1);
		...
		for(int j=0;j<10000;j++){
			...
			Integer x = snap.get(j);
			if(x==null)
				System.out.println("snap sec get " + j +" is null");
			else if(x!=j)
				System.out.println("snap sec get " + j +" is not x "+x);
			//Integer y=tree.get(j);
			/* if(y!=j+1)
						System.out.println(y+" (tree) is different than "+(j+1)); */
		}  
